...
| Data | 15/09/2026 |
| Status | Núcleo jogável de ponta a ponta (backend + app), incluindo equipamento (RF-08, RF-37, RF-38, RF-41; RN-23 a RN-25) e navegação global entre as quatro telas (RF-57) |
...
| **Q-07** | Empunhar duas armas (RN-25) concede uma ação de ataque extra por tick no combate ATB, ou só acumula os bônus de atributo de ambas via atributos efetivos (RF-08)? RF-08 já existe (v2.3), mas `SpeedBasedCombatResolver` continua consultando só `attributes()` (base) — decisão deliberada para manter o módulo de equipamento (v2.3) focado em equipar/desequipar e não também mexer no resolvedor de combate. | RF-08, RF-21, RN-25, UC-03 | Definir antes de fazer o combate reagir a itens equipados (fora do escopo imediato do módulo de equipamento em si). |
